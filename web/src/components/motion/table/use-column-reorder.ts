...
export function useColumnReorder<T>({
  ...
  const orderedColumns = useMemo(() => {
    ...
    columns.forEach((column, i) => {
      ...
      const previousIndex = previous ? resultKeys.indexOf(previous.key) : -1;
      const at = i === 0 ? 0 : previousIndex === -1 ? i : previousIndex + 1;
  ...
  const dropIndexFor = useCallback(
    (clientX: number) => {
      for (const [i, column] of orderedColumns.entries()) {
        const rect = thRefs.current[column.key]?.getBoundingClientRect();
