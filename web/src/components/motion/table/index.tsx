...
/** Read the root font size without assuming browser layout during SSR. */
function readRootFontSize() {
  const measured = Number.parseFloat(
    getComputedStyle(document.documentElement).fontSize,
  );
  return measured > 0 ? measured : DEFAULT_ROOT_FONT_SIZE;
}
function subscribeRootFontSize(onChange: () => void) {
  window.addEventListener('resize', onChange);
  return () => window.removeEventListener('resize', onChange);
}
function useRootFontSize() {
  return useSyncExternalStore(
    subscribeRootFontSize,
    readRootFontSize,
...
export function Table<T>({
  ...
  'use no memo'; // TanStack Virtual exposes mutable, imperative measurements.
  ...
  const paddingBottom = virtualized && lastItem ? totalSize - lastItem.end : 0;
              ...
              <>
                ...
                {virtualItems.map((vItem) => {
                  ...
                  return (
                    ...
                    >
                      {selectable ? (
                        <td className="text-center" aria-colindex={1}>
