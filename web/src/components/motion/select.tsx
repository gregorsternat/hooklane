...
export function Select({
  ...
  function onKeyDown(event: KeyboardEvent<HTMLDivElement>) {
    ...
    if (open && (event.key === 'Enter' || event.key === ' ')) {
      const option = enabledOptions().find((item) => item === event.target);
      if (option) {
        event.preventDefault();
        option.click();
      }
    ...
    if (!open) {
      ...
      initialFocus.current = hasSelection
        ? 'selected'
        : event.key === 'ArrowUp'
          ? 'last'
          : 'first';
  ...
  // The keyboard handler stays inside the field so Escape closes its panel
  // without also dismissing an enclosing dialog.
  useEffect(() => {
    if (!open) return;
...
export type SelectTriggerProps = Omit<
  ...
  'id' | 'disabled' | 'role' | 'type' | 'ref' | 'children'
> & { children: ReactNode };
...
export function SelectTrigger({
  ...
  return (
    <motion.button
      {...props}
      ref={(node) => ctx.setTriggerElement(node)}
      type="button"
      id={ctx.triggerId}
      role="combobox"
